...
import { getItems, getDomain } from "./fetch";
...
export const fetchCategoriesProductColumns = async (
  baseCategory: string | number,
  series: string | number
) => {
  try {
    const url = `${getDomain()}/erpapi/panel/schema/columns?BaseCategory=${baseCategory}&Series=${series}`;
...
export const fetchCategoryProducts = async (
  ...
  const payload: IGetItemPayload = {
    ...
    JToken: {
      Category: baseCategory.toString(),
      Series: lastId.toString(),
